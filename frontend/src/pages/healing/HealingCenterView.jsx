...
import { Wrench, RefreshCw, Brain, CheckCircle, XCircle, Zap, Ticket, ExternalLink, Info, Sparkles, RotateCcw, Code2, AlertTriangle, Lock, Unlock } from 'lucide-react';
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            {hasJira && action.jira_summary && (
              <p className="text-xs text-gray-500 truncate mt-0.5">Story: {action.jira_summary}</p>
            )}
          ...
          <div className="text-xs text-gray-600 flex-shrink-0 text-right">
            {action.test_case_module && <span className="block capitalize text-gray-500">{action.test_case_module}</span>}
            {action.tokens_used > 0 && <span className="block">{action.tokens_used} tokens</span>}
          </div>
        </div>
      </div>
...
function LocatorStateBadge({ state }) {
  if (state === 'healed') {
    return <span className="inline-flex items-center gap-1 text-xs text-green-400 bg-green-500/10 border border-green-500/20 px-2 py-0.5 rounded-full"><Unlock className="w-3 h-3" /> Fixed</span>;
  }
  if (state === 'broken') {
    return <span className="inline-flex items-center gap-1 text-xs text-red-400 bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded-full"><Lock className="w-3 h-3" /> Broken (armed)</span>;
  }
  return <span className="inline-flex items-center gap-1 text-xs text-gray-400 bg-gray-500/10 border border-gray-500/20 px-2 py-0.5 rounded-full">Working (DEMO_MODE off)</span>;
}

export default function HealingCenterView() {
  ...
  const load = async (silent = false) => {
    ...
    try {
      const [actionsRes, demoRunsRes, locatorsRes] = await Promise.allSettled([
        healingApi.getActions({ limit: 100 }),
        demoHealingApi.getRuns({ limit: 20 }),
        demoHealingApi.getLocators(),
      ]);
      ...
      if (locatorsRes.status === 'fulfilled') setLocators(locatorsRes.value?.data || []);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetDemo = async () => {
    ...
    try {
      ...
      // Update immediately from the response — don't wait on the next poll,
      // and don't rely on demoRuns changing (a reset touches locator state,
      // not run history, so without this the screen would look unchanged).
      setLocators(res.data || []);
      setResetMessage({ type: 'success', text: `Reset ${res.data?.length || 0} locator(s) back to broken — ready for another demo run.` });
    } catch (err) {
  ...
  if (loading) return <div className="py-16 text-center text-gray-500 text-sm">Loading healing data...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Demo Locators status — always visible so Reset has something to change on screen */}
      {locators.length > 0 && (
        <div className="glass-card p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-white flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-purple-400" /> Demo Locators
            </h2>
            <button
          ...
          <div className="space-y-2">
            {locators.map((loc) => (
              <div key={loc.locatorKey} className="flex items-center justify-between bg-gray-800/40 rounded-lg px-3 py-2">
                <span className="text-xs font-mono text-gray-300">{loc.locatorKey}</span>
                <LocatorStateBadge state={loc.state} />
              </div>
            ))}
          </div>
          {resetMessage && (
            <p className={clsx('text-xs mt-2', resetMessage.type === 'success' ? 'text-green-400' : 'text-red-400')}>
              {resetMessage.text}
            </p>
          )}
        </div>
          ...
          <div className="p-4 border-b border-gray-800 flex items-center justify-between gap-3">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                jiraOnly ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'
              }`}
